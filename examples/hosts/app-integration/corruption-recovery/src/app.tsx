...
async function cleanRecoveryAgent(container: IFluidContainer): Promise<void> {
    const recMap = container.initialObjects.recoveryMap as SharedMap;
...
async function getRecoveryAgent(
    ...
    const recMap = container.initialObjects.recoveryMap as SharedMap;
    let recoveryContainerId = recMap.get("recoverContainerId");
    ...
    return await RecoveryAgent.getRecoveryAgent(
        recoveryContainerId,
        containerSchema
    );
}
...
async function start(): Promise<void> {
    ...
    const recoverContainer = async () => {
        const recDocId = await recoveryAgent?.recoverDoc((container) => {
            cleanRecoveryAgent(container);
        });
